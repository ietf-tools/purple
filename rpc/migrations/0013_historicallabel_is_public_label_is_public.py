...
def set_existing_labels_public(apps, schema_editor):
    # Existing labels were all public before this flag; the default (False)
    # applies only to labels created afterwards. Record a System history entry
    # per label (migrations bypass simple-history signals) so the change is
    # attributed to "(System)" and doesn't surface on a later editor's save.
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(set_existing_labels_public, migrations.RunPython.noop),
    ]

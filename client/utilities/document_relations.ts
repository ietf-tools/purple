...
const font_family = '"Inter",system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue","Noto Sans","Liberation Sans",Arial,sans-serif,"Apple Color Emoji","Segoe UI Emoji","Segoe UI Symbol","Noto Color Emoji"'
...
// FIXME: change 'some group' that's a placeholder type
type Group = '' | 'none' | "this group"
...
type Node = {
    id: string
    url: string
    level?: "" | "Informational" |  "Experimental" | "Proposed Standard" | "Best Current Practice"
    group?: Group
    rfc?: number
    replaced?: boolean
    dead?: boolean
...
export function draw_graph(data: Data, group: Group) {
    ...
    svgElement.className = "w-100 h-100"
    ...
    const a = node.append("a")
        .attr("href", d => d.url)
        .attr("title", d => {
          const nodePropsWeCareAbout: (keyof Node)[] = ["replaced", "dead", "expired"]
            let type = nodePropsWeCareAbout.filter(x => d[x])
                .join(" ");
            if (type) {
                type += " ";
            }

  ...
  <div>
    ...
    <form class="flex flex-row items-center gap-6 pt-5" novalidate>
      <div class="flex flex-row items-center gap-1">
        <label class="text-gray-900 dark:text-white text-sm font-bold mr-1" for="filterInput"
          >Filter:</label
        >
        <input
          type="text"
      ...
      <RpcCheckbox
        ...
        @change="showInactive = !showInactive" />
    </form>
    ...
    <div class="flex flex-col gap-3 mt-5 mb-10">
      <div
        ...
        <h2
          class="flex flex-row items-start text-lg whitespace-nowrap grow-0 shrink-0 w-[7em] px-2">
        ...
        <ul class="flex flex-wrap pt-1 gap-y-1 gap-x-3 text-xs leading-[1]">
          <li
            ...
            v-for="(document, index) in cluster.documents"
            :key="index">
            <DocumentCardMini :document="document" />
          </li>
          <li
            class="whitespace-nowrap inline-flex items-center gap-1"
            v-for="name in notReceivedRefs(cluster)"
            :key="`notreceived-${name}`">
...
const {
  ...
  pending,
  refresh
} = await useAsyncData('all-clusters', () => api.clustersList(), {
  server: false,
  lazy: true
})
...
const clusterSearch = computed(() =>
  clusters.value ? clusters.value.map((cluster) => JSON.stringify(cluster)) : []
)
...
function notReceivedRefs(cluster: Cluster): string[] {
  ...
  for (const doc of cluster.documents ?? []) {
    for (const ref of doc.references ?? []) {
      if (
        ref.relationship === 'not-received' &&
        ref.targetDraftName &&
        !memberNames.has(ref.targetDraftName)
      ) {
...
const filteredClusters = computed(() =>
  clusters.value
    ? clusters.value
        .filter((_cluster, index) =>
          clusterSearch.value[index]
            ? clusterSearch.value[index].includes(filterValueString.value.trim())
            : false
        )
        .filter((cluster) => (showInactive.value === false ? cluster.isActive : true))

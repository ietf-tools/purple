    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <BaseCard class="w-full grid place-items-stretch">
        ...
        <p class="mt-1 text-xs opacity-60">
          Counts of documents and pages moving through the queue each period. Page counts are summed
          ...
          <table
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-neutral-800">
              ...
              <tr v-for="(row, i) in docCountRows" :key="`doc-${i}`">
                <th
                  scope="row"
                  class="py-2 pl-6 pr-3 text-left font-normal"
                  :class="row.divider ? dividerClass : ''">
                  {{ row.label }}
                  <span v-if="row.note" class="block whitespace-normal text-xs opacity-60">{{
                    row.note
                  }}</span>
                </th>
                <td
...
import {
  StatsQueuePeriodEnum,
  type QueueCountStatPeriod,
  type QueueCountStats
} from '~/purple_client'
import { formatWeekRange, isWeekLabel } from '~/utils/statsViz'
...
// displayed data (shared helpers in statsViz).
...
const docCountRows: MetricRow[] = [
  ...
  {
    label: 'Docs entering with missing references',
    get: (p) => p.docsEnteredMissingRef,
    format: int
  },

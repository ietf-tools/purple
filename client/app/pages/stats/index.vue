    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <BaseCard class="w-full grid place-items-stretch">
        ...
        <p class="mt-1 text-xs opacity-60">
          Each bin covers the documents in the queue during that period; bars show the time spent in
          each assignment role <em>within</em> the period (a per-period flow, not a running
          cumulative total).
        </p>
        ...
        <div class="mt-3 flex items-center justify-end gap-2 text-xs">
          ...
          <div
            class="inline-flex overflow-hidden rounded-md border border-gray-300 dark:border-neutral-600">
            <button
              type="button"
              class="px-2 py-1"
              :class="
                mode === 'calendar' ? 'bg-violet-600 text-white' : 'bg-white dark:bg-neutral-800'
              "
              @click="mode = 'calendar'">
              Calendar days
            </button>
            <button
              ...
              :class="
                mode === 'working' ? 'bg-violet-600 text-white' : 'bg-white dark:bg-neutral-800'
              "
              @click="mode = 'working'">
              Working days
            </button>
            <button
              ...
              class="px-2 py-1 border-l border-gray-300 dark:border-neutral-600"
              :class="
                mode === 'share' ? 'bg-violet-600 text-white' : 'bg-white dark:bg-neutral-800'
              "
              @click="mode = 'share'">
              Share %
            </button>
      ...
      <BaseCard class="mt-8 w-full grid place-items-stretch">
        ...
        <div v-if="periods.length === 0" class="px-3 py-3 text-center text-sm opacity-60">
          No data.
        </div>
          ...
          <table
            ...
            <thead class="bg-gray-50 dark:bg-neutral-800">
              <tr>
                ...
                <th
                  ...
                  <div
                    v-if="p.legacyIncluded"
                    class="text-xs font-normal text-violet-500"
                    title="Includes pre-transition labeled states">
                    legacy
                  </div>
                </th>
              </tr>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-neutral-800">
              ...
              <tr
                ...
                <th
                  scope="row"
                  class="py-2 pl-4 pr-3 text-left font-semibold text-red-600 dark:text-red-400">
                  Blocked
                </th>
                <td
                  v-for="p in periods"
              ...
              <tr v-for="role in blockedRoles" :key="role">
                <th
                  scope="row"
                  class="py-2 pl-8 pr-3 text-left font-normal text-red-600 dark:text-red-400">
                  {{ role }}
                </th>
                <td v-for="p in periods" :key="p.label" class="px-3 py-2 text-right tabular-nums">
                  {{ cellValue(p, secondsFor(p, role)) }}
        ...
        <p v-if="mode === 'working'" class="mt-2 px-1 text-xs opacity-60">
          Working days are calculated assuming an average of {{ workingHoursPerYear }} working hours
          per year.
        </p>
      </BaseCard>
    </div>
...
const notBlockedRoles = computed(() =>
  roleColumns.value.filter((r) => !blockedRoleSet.value.has(r))
)

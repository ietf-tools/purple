  ...
  <div class="w-full">
    <div ref="root" class="relative">
      <div ref="container" class="w-full overflow-x-auto text-gray-600 dark:text-neutral-300">
        <svg
          ...
          class="block"
          role="img"
          aria-label="Stacked bar chart of time spent per assignment role each period. The same data is in the table below." />
    ...
    <div v-if="categories.length" class="mt-3 text-xs">
      <p class="mb-1 opacity-50">
        Click to hide a category; the chart rescales. Minor roles are grouped as “Other”.
      </p>
...
const categories = computed<Category[]>(() => {
  ...
  const out: Category[] = []
  const take = (
    list: { role: string; seconds: number }[],
    max: number,
    palette: string[],
    isBlocked: boolean,
    otherColor: string
...
function secondsFor(p: QueuePeriodStat, cat: Category): number {
  return cat.members.reduce(
    (sum, role) => sum + (p.byRole.find((r) => r.role === role)?.seconds ?? 0),
    0
  )
}
  ...
  const yAxis = d3
    ...
    .tickFormat((d) =>
      isShare
        ...
        : `${Math.round((Number(d) / SECONDS_PER_DAY) * dayScale)}d`
    )
  ...
  data.forEach((d) => {
    ...
    for (const cat of cats) {
      ...
      if (h > 0) {
        ...
        const attach = (sel: d3.Selection<never, unknown, null, undefined>) =>
          sel
            .attr('fill', cat.color)
        ...
        if (cat === topCat) {
          attach(
            svg.append('path').attr('d', roundedTopRect(bx, yTop, bandW, h, CORNER_PX)) as never
          )
        } else {
          attach(
            svg

...
const finishAssignment = async () => {
  ...
  try {
    ...
    if (updatedAssignment.state !== 'done') {
      console.error(
        "Server response to update assignment to 'done' wasn't set to 'done'. Was: ",
        updatedAssignment
      )
...
const patchTimeSpent = async () => {
  ...
  try {
    ...
    if (updatedAssignment.timeSpent !== timeSpent) {
      console.warn(
        'potential time spent translation bug',
        timeSpent,
        '!==',
        updatedAssignment.timeSpent,
        ' from ',
        updatedAssignment
      )
    }
    ...
    snackbarForErrors({
      snackbar,
      defaultTitle: 'Unable to update time spent on assignment',
      error: e
    })
  }
  props.onSuccess() // triggers reload of data from page under modal

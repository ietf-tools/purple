  ...
  <HeadlessTransitionRoot as="template" :show="isShown">
    <HeadlessDialog as="div" class="relative z-50" @close="close">
      ...
      <HeadlessTransitionChild
        ...
        leave-to="opacity-0">
        <div
          class="fixed inset-0 bg-gray-500 dark:bg-black bg-opacity-75 dark:bg-opacity-50 transition-opacity backdrop-blur-sm" />
      </HeadlessTransitionChild>
      <!-- MODE: Side -->
      ...
      <div v-else class="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:px-20 md:py-10">
        <HeadlessTransitionChild
          ...
          <HeadlessDialogPanel
            class="mx-auto h-full transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white dark:bg-neutral-800 shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
            <component :is="opts.component" v-bind="opts.componentProps" />
          </HeadlessDialogPanel>
        </HeadlessTransitionChild>
      </div>

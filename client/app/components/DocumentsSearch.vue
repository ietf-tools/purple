<template>
  ...
  <div
    ...
    <label
      class="text-gray-900 dark:text-gray-200 w-[160px] text-right text-sm font-bold mr-1"
      :for="props.id">
      {{ props.label }}:</label
    >
    <div>
        ...
        <ComboboxAnchor
          ...
          <span
            ...
            <button
              ...
              :label="`Unselect document ${selectedRfcToBe.name}${selectedRfcToBe.rfcNumber ? `(RFC ${selectedRfcToBe.rfcNumber})` : ''}`">
              &times;
            </button>
          </span>
          <ComboboxInput
...
watch(debouncedInputRef, async () => {
  ...
  try {
    const result = await api.documentsSearch(
      { q: debouncedInputRef.value },

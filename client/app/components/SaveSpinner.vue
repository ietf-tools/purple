<template>
  <TooltipProvider>
    <TooltipRoot>
      <TooltipTrigger
        type="button"
        :style="`font-size:${props.size}; width: ${props.size}; height: ${props.size}`"
        class="text-black dark:text-white relative">
        <Icon
          v-if="throttledStatus === 'pending'"
          name="ei:spinner-3"
          :size="props.size"
          class="absolute top-0 left-0 animate-spin" />
        <Icon
          v-else-if="throttledStatus === 'success'"
          name="heroicons:check"
          :size="props.size"
          class="absolute top-0 left-0" />
        <Icon
          v-else-if="throttledStatus === 'error'"
          name="ei:exclamation"
          :size="props.size"
          class="absolute top-0 left-0" />
      </TooltipTrigger>
      <TooltipPortal>
        <TooltipContent
          ...
          <span v-else-if="throttledStatus === 'error'"> Error: {{ saveStatusError }} </span>
        </TooltipContent>
      </TooltipPortal>
    </TooltipRoot>
  </TooltipProvider>

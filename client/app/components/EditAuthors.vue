    ...
    <div v-if="draft.authors" class="border-5 border-gray-700 text-gray-500">
      ...
      <ul ref="parent" class="block min-w-[200px]">
        <li
          ...
          :index="index"
          :key="author.id"
          class="flex items-center justify-between pl-2 cursor-ns-resize pr-1 py-1 mt-1 border rounded-md border-gray-400">
          ...
          <input
            ...
            class="flex-1 min-w-0 cursor-text border border-gray-400 rounded py-1 mr-2" />
          <label
            class="border flex gap-2 items-center cursor-pointer border-gray-400 inline-block px-2 py-1 rounded text-sm mr-4">
            editor?
            <input
              type="checkbox"
              class=""
              v-model="author.isEditor"
              @change="() => patchAuthor(author)" />
          </label>
          <button
            type="button"
            @click="handleRemoveAuthor(index)"
            class="border-1 cursor-not-allowed border-gray-500 rounded px-2 py-1 hover:bg-gray-200 focus:bg-gray-200">
            &times;
          </button>
        </li>
      </ul>
...
const authorsRef = computed({
  ...
  set: (val) => {
    if (draft.value) draft.value.authors = val
  }
})
...
watch(
  () => draft.value?.authors?.map((author) => author.id),
  async (authorIds) => {
    if (!authorIds) return

    const existingAuthorIds = authorIds.filter((maybeId) => maybeId !== undefined)

    ...
    <div class="flex justify-between gap-x-4">
      <div v-if="comment.by" class="py-0.5 text-xs leading-5 text-gray-500">
        <span
          ...
          :title="
            comment.by.rpcperson ? `${comment.by.name} (user #${comment.by.rpcperson})` : undefined
          ">
          {{ comment.by.name }}
        </span>
        commented
        <time v-if="comment.time" :datetime="comment.time.toISOString()" class="text-gray-500">
          {{ comment.ago }}
        </time>
        <span
          v-if="comment.lastEdit && comment.lastEdit.by"
          :title="
            comment.lastEdit.by.personId
              ? `${comment.lastEdit.by.name} (${comment.lastEdit.by.email ?? ''})`
              : undefined
          ">
          (last edited
      ...
      <div class="flex-none py-0.5 text-xs leading-5">
        <button
          ...
          <Icon
            name="circum:edit"
            class="w-5 h-5 no-underline text-gray-600 hover:text-indigo-900 cursor-pointer" />
        </button>
      </div>
    </div>
